...
class MentorProfileController {
  ...
  async createMyProfile(req, res) {
    try {
      ...
      return res.status(201).json(new ApiResponse(201, 'Mentor profile created successfully', { profile, user: mappedUser }));
    } catch (error) {
      const statusCode = getStatusCode(error);
      return res.status(statusCode).json({
        success: false,
  ...
  async updateMyProfile(req, res) {
    try {
      ...
      return res.status(200).json(new ApiResponse(200, 'Mentor profile updated successfully', { profile, user: mappedUser }));
    } catch (error) {
      const statusCode = getStatusCode(error);
      return res.status(statusCode).json({
        success: false,

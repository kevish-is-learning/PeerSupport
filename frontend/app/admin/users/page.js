...
export default function AdminUsersPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8">
      {/* Header */}
      <header>
        <h1 className="text-3xl font-light tracking-tight text-white">Users</h1>
        <p className="mt-2 text-sm text-zinc-400">Search, filter, and manage all registered users.</p>
      </header>
      ...
      <section className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="min-w-[200px] flex-1 rounded-xl border border-zinc-800 bg-[#0a0a0a] px-4 py-2.5 text-sm text-white outline-none placeholder:text-zinc-600 focus:border-zinc-600 transition-colors"
        />
        <select
          value={roleFilter}
                ...
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-zinc-900/20 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <p className="mt-0.5 text-xs text-zinc-500">{u.email} · {u.provider}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-block rounded-full border px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider ${ROLE_COLORS[u.role] || "border-zinc-800 text-zinc-400 bg-zinc-900/50"}`}>
                        {u.role}
                      </span>
                      {!u.hasProfile && u.role !== "ADMIN" && (
                        <p className="mt-1.5 text-[10px] text-amber-500/80">Incomplete Profile</p>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-block rounded-full border px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider ${u.isActive ? "border-emerald-900/50 bg-emerald-950/20 text-emerald-400" : "border-red-900/50 bg-red-950/20 text-red-400"}`}>
        ...
        {pagination.totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-zinc-800 px-6 py-4">
            <button
              ...
              className="rounded-lg border border-zinc-800 bg-black px-4 py-2 text-xs font-medium text-white hover:bg-zinc-900 disabled:opacity-40 transition-colors"
            >
              Previous
            </button>

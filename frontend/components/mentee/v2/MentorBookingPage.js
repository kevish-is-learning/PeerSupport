...
import {
  Loader2, Clock, Calendar, Check, ChevronDown, ChevronUp,
} from "lucide-react";
...
export default function MentorBookingPage({ mentorProfileId }) {
  ...
  const getServiceAvailableDates = () => {
    ...
    return mentor.availability
      .filter((a) => a.specificDate && a.services?.some((s) => s.mentorServiceId === selectedService.id))
      .map((a) => a.specificDate);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border-2 border-black bg-white p-6"
        ...
        <p className="text-sm text-gray-500">Select a service to view available dates and time slots</p>
      </div>
      ...
      <div className="space-y-4">
        {mentorServices.map((svc) => {
          ...
          return (
            <div key={svc.id} className="rounded-2xl border-[3px] border-black bg-white overflow-hidden"
              style={{ boxShadow: isExpanded ? "5px 5px 0 0 #5061E4" : "4px 4px 0 0 #d1d5db" }}>
              {/* Service Header */}
              <button
                ...
                <div className="text-left">
                  <h4 className="text-base font-bold text-gray-900">{svc.label || svc.title}</h4>
              ...
              {isExpanded && (
                <div className="border-t-2 border-gray-200 px-5 py-5 space-y-5">
                  {/* Date Selection */}
                  <div>
                    <h5 className="text-sm font-extrabold text-gray-900 mb-3">Select a Date</h5>
                      ...
                      {dateRange.filter((date) => {
                        ...
                        return (
                          <button
                            ...
                            style={{ boxShadow: isSelected ? "3px 3px 0 0 #000" : "2px 2px 0 0 #d1d5db" }}>
                            {date.toLocaleDateString("en-IN", { month: "short", day: "numeric", weekday: "short" })}
                          </button>
                        );
                      })}
                      {dateRange.filter((d) => serviceAvailDates.includes(d.toISOString().split("T")[0])).length === 0 && (
                        <p className="col-span-full text-sm text-gray-400 text-center py-4">No available dates for this service</p>
                      )}
                    </div>
                  </div>
                  ...
                  {selectedDate && (
                    <div>
                      <h5 className="text-sm font-extrabold text-gray-900 mb-3">Select a Time Slot</h5>
                      {slotsLoading ? (
